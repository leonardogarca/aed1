...
void mostrarNivel(No* no, int nivel, int nivel_atual){
    if (no == NULL){
        return;
    }
    ...
    mostrarNivel(no->esquerdo, nivel, nivel_atual + 1);
    mostrarNivel(no->direito, nivel, nivel_atual + 1);
    return;
}
